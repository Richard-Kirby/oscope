...
digital_in_BCM = [22, 27, 4, 17, 14, 23, 24, 13, 19]

for i in range(8):
    # Pin Setup:
    GPIO.setup(digital_in_BCM[i], GPIO.IN) # Button pin set as input w/ pull-up
...
for i in range(8):
    pi.set_mode(digital_in_BCM[i], pigpio.INPUT)
...
try:
    ...
    while True:
        ...
        for i in range(2):
            digital_in[i]= GPIO.input(digital_in_BCM[i])

        #print (values[0], values[1])

        #print(values[0])

        samples.append((time.time(), *values, *digital_in))
        sample_count +=1
    ...
    with open('/home/pi/oscope/oscope.csv', 'w', newline='') as resultfile:
        ...
        for sample in samples:
            str = "{}, {:.3f}, {:.3f}, {}, {}\n".format(sample_num, sample[1], sample[2], sample[9], sample[10])
